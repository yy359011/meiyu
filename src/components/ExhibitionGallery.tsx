...
export default function ExhibitionGallery({ onSelectItem, onViewAll }: ExhibitionGalleryProps) {
  ...
  const filteredData = activeFilter === '全部'
    ...
    : GALLERY_DATA.filter(item => item.category === activeFilter).slice(0, 6);
  ...
  return (
    <section className="py-14 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto scroll-mt-20" id="art-gallery-section">
      ...
      {/* Filter Menu Row */}
      <div className="flex flex-wrap items-center gap-2 mb-8" id="gallery-filters">
        {filters.map((filter) => (
          <button
            key={filter}
        ...
        <AnimatePresence mode="popLayout">
          {filteredData.map((item) => (
            ...
            >
              ...
              <div className="absolute top-4 left-4 bg-white/10 backdrop-blur-md px-2.5 py-1 rounded text-[9px] text-[#fef08a] uppercase tracking-wider font-semibold z-20 shadow-sm border border-white/15">
                {item.category}
              </div>

              {/* Award seal badge icon absolute top right */}
              <div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="w-8 h-8 rounded-full bg-amber-500 text-white flex items-center justify-center shadow-lg" title="年度优秀收藏奖">
