...
const INITIAL_EVENTS_DATA = [
  {
    ...
    dateLabel: '报名截止',
    dateText: '2026年6月30日',
    coverUrl: new URL('../assets/images/calligraphy_brush_1780883968133.png', import.meta.url).href,
    tag: '书法',
    mode: '线上' as '线上' | '线下',
  ...
  },
  {
    ...
    statusLabel: '即将开始',
    title: '"丝路新语"青年非遗创新设计周',
    dateLabel: '活动开始',
    dateText: '2026年7月10日',
    coverUrl: new URL('../assets/images/exhibition_hall_1780883993826.png', import.meta.url).href,
    tag: '非遗创新',
    mode: '线上' as '线上' | '线下',
    description: '以古丝绸之路文化遗产为灵感，邀请青年设计师结合数字技术与传统工艺，探索非遗的当代表达。活动将开设线上设计工作室、专家直播评审、以及获奖作品数字展厅等环节。',
    rules: '1. 参赛者需为45岁以下青年设计师及学生；2. 需提交完整设计方案与作品说明；3. 作品需至少融入一项非遗元素；4. 每人限投2件作品。',
    startDate: '2026年7月10日',
    endDate: '2026年7月20日',
    location: '线上报名通道 / 非遗数字化研习中心',
    guarantee: '优秀获奖作品将进入公共美育数字展厅，并获得非遗传承大师现场指导机会。',
    submissionFile: null as any,
    submissionTitle: '',
    progressStep: 1,
  },
...
export default function EventsPage({ onSelectEvent, onBackToHome, onEnroll, onViewRecords, onViewDetail }: { onSelectEvent?: (ev: any) => void, onBackToHome: () => void, onEnroll?: (eventTitle: string) => void, onViewRecords?: (eventTitle: string) => void, onViewDetail?: (eventTitle: string, submissionTitle: string, status: 'submitting' | 'reviewing') => void }) {
  const [events, setEvents] = useState(INITIAL_EVENTS_DATA);
  const [selectedStatusTab, setSelectedStatusTab] = useState<'all' | 'registering' | 'upcoming' | 'ended'>('all');
  ...
  const [certificateList, setCertificateList] = useState<{ title: string; date: string }[]>([]);
  ...
  const getStatusBadgeConfig = (status: string) => {
    switch (status) {
      ...
      case 'upcoming':
        return { label: '即将开始', bg: 'bg-sky-50 text-sky-700 border-sky-200/60' };
      case 'ended':
  ...
  const handleEnrollSubmit = () => {
    if (onEnroll && activeDetailEvent) {
      onEnroll(activeDetailEvent.title);
    }
  };
  ...
  return (
    <div className="w-full bg-[#fbf9f4] min-h-screen text-stone-800 pb-16">
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(10, 12, 18, 0.5), rgba(10, 12, 18, 0.7)), url('https://images.unsplash.com/photo-1577720643272-265f09367456?q=80&w=1600&auto=format&fit=crop')`
        }}
      >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1.5 min-w-0">
                        <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                          ...
                          <span className={`text-[10px] font-semibold tracking-wider font-serif px-2 py-0.5 rounded ${event.mode === '线上' ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-green-50 text-green-600 border border-green-200'}`}>
                            {event.mode}
                          </span>
          ...
          <div className="lg:col-span-5">
              ...
              {activeDetailEvent ? (
                      ...
                      {activeDetailEvent.status === 'registering' && (
                        ...
                        >
                          <button
                            ...
                            className="flex-1 py-3 rounded-lg text-sm font-semibold tracking-widest text-white bg-[#b11e22] hover:bg-[#99161a] transition-all shadow-md hover:shadow-lg cursor-pointer flex items-center justify-center"
                          >
                            {isEnrolling ? (
                              <>
                          ...
                          <button
                            onClick={() => onViewRecords && onViewRecords(activeDetailEvent.title)}
                      ...
                      {activeDetailEvent.status === 'submitting' && (
                        ...
                        >
                          <div className="flex items-center space-x-2 text-blue-800 border-b border-blue-100 pb-1.5 mb-1.5 select-none">
                            ...
                            <span className="text-xs font-serif font-bold">作品提交中</span>
                          </div>
                          ...
                          <button
                            onClick={() => onViewDetail && onViewDetail(activeDetailEvent.title, activeDetailEvent.submissionTitle || '我的精选作品方案', 'submitting')}
                            className="w-full py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-serif text-xs font-bold rounded cursor-pointer transition-colors flex items-center justify-center space-x-2"
                          >
                      ...
                      {activeDetailEvent.status === 'reviewing' && (
                        ...
                        >
                          <div className="flex items-center space-x-2 text-amber-800 border-b border-amber-100 pb-1.5 mb-1.5 select-none">
                            ...
                            <span className="text-xs font-serif font-bold">作品审核中</span>
                          </div>

                          <p className="text-xs text-stone-600 leading-relaxed font-serif">
                            您的作品已提交成功，正在等待专家审核，请耐心等待。
                          </p>

                          <button
                            onClick={() => onViewDetail && onViewDetail(activeDetailEvent.title, activeDetailEvent.submissionTitle || '我的精选作品方案', 'reviewing')}
                            className="w-full py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-serif text-xs font-bold rounded cursor-pointer transition-colors flex items-center justify-center space-x-2"
                          >
                            <Eye className="w-4 h-4" />
                            <span>查看详情</span>
                          </button>
                        ...
                        >
                          ...
                          crossOrigin="anonymous"
                            ...
                            <p className="text-[11px] leading-relaxed text-stone-600 font-serif">
                              恭喜同学！作品 <strong>《{activeDetailEvent.submissionTitle || '大国重构创新'}》</strong> 凭借突出的美学表现，正在数字化陈列中。
                            </p>
                ...
                >
                  ...
                  <p className="text-stone-400 text-xs max-w-xs mt-2 font-serif leading-relaxed">
                    点击左侧任意项，即可实时展示该项美育活动的「报名」、「提报作品」及「成就证书」全景页面。
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="mt-6 bg-stone-900 text-stone-100 p-6 sm:p-7 rounded-2xl border border-stone-800 text-left relative overflow-hidden">
              ...
              <div className="space-y-4 max-w-lg relative z-10">
                ...
                <div className="flex gap-2.5 pt-1.5">
                  <input
                    ...
                    placeholder="请输入学号或姓名，例如：20260001 或 张美育"
                  />
                  <button
                    onClick={() => {
                      if (certificateName.trim()) {
                        const mockCertificates = [
                          { title: '"七下西洋"大明宝船水密隔舱营造法度结业证明', date: '2026年5月30日' },
                          { title: '美育中心优秀作品《我的精选作品方案》金榜特藏荣誉证书', date: '2026年6月8日' },
                          { title: '美育中心书法艺术实践优秀学员证书', date: '2026年4月15日' },
                        ];
      ...
      <AnimatePresence>
        {showCertificateList && (
          ...
          >
            ...
            >
              {/* Header */}
              <div className="bg-gradient-to-r from-[#b11e22] to-[#921417] px-6 py-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <Award className="w-6 h-6 text-white" />
                    <div>
                      <h3 className="font-serif text-lg font-bold text-white">荣誉证书查询结果</h3>
                      <p className="text-xs text-white/70 mt-0.5">学员：{certificateName}</p>
                    </div>
                  </div>
                  <button
                ...
                {certificateList.length > 0 ? (
                  <div className="space-y-4">
                    {certificateList.map((cert, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-4 bg-amber-50/50 border border-amber-100 rounded-lg hover:bg-amber-50 transition-colors"
                      >
                        <div className="flex-1">
                          <p className="text-sm font-medium text-stone-800">{cert.title}</p>
                          <p className="text-xs text-stone-500 mt-1">发证时间：{cert.date}</p>
                        </div>
                        <div className="flex items-center space-x-2 ml-4">
                          <button
                            onClick={() => {
                              setCertificateDetail(cert);
                              setShowCertificate(true);
                              setShowCertificateList(false);
                            }}
                            className="px-4 py-2 bg-[#b11e22] hover:bg-[#921417] text-white text-xs font-medium rounded-lg transition-colors cursor-pointer flex items-center space-x-1.5"
                          >
                          ...
                          <button
                            onClick={() => {
                              ...
                              link.href = cert.content;
                              link.download = `${cert.title}.jpg`;
                              document.body.appendChild(link);
                              link.click();
                              document.body.removeChild(link);
                            }}
                            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium rounded-lg transition-colors cursor-pointer flex items-center space-x-1.5"
                          >
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-stone-500">暂无荣誉证书记录</p>
                  </div>
                )}
              </div>
              ...
              <div className="px-6 py-4 bg-stone-50 border-t border-stone-100 flex justify-end">
                <button
                  onClick={() => setShowCertificateList(false)}
                  className="px-6 py-2 text-sm text-stone-600 hover:text-stone-800 border border-stone-200 rounded-lg hover:bg-white transition-colors cursor-pointer"
                >
                  关闭
                </button>
              </div>
