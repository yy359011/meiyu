/**
 * UploadWorkPage - 原创作品上传页面
 */

import { useState, useRef, useEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import {
  ArrowLeft, Upload, XCircle, ChevronRight, ChevronDown, Save, FileText,
  Eye, Check, FileVideo, FileImage, FileAudio, File, X, Plus
} from 'lucide-react';

// ============ Types ============
interface UploadWorkPageProps {
  onBack: () => void;
...
// ============ Constants ============
const SUB_CATEGORIES = ['审美实践', '优秀作品', '获奖作品', '校园美育'];

const CATEGORY_OPTIONS: Record<string, string[]> = {
  '审美实践': ['美术', '书法', '音乐', '戏曲', '舞蹈', '摄影', '设计'],
  '优秀作品': ['美术', '书法', '音乐', '戏曲', '舞蹈', '摄影', '设计'],
  '获奖作品': ['美术', '书法', '音乐', '戏曲', '舞蹈', '摄影', '设计'],
  '校园美育': ['美术', '书法', '音乐', '戏曲', '舞蹈', '摄影', '设计'],
};

let fileIdCounter = 0;
const genFileId = () => `file-${Date.now()}-${++fileIdCounter}`;

// ============ Component ============
export default function UploadWorkPage({ onBack, onSubmit }: UploadWorkPageProps) {
  ...
  const handleSubmit = () => {
    if (!form.title || !form.category || !form.subCategory) return;
    
    const allFiles = Object.values(uploadedFiles).flat();
    const formats = allFiles.map(f => {
  ...
  const isFormValid = form.title && form.subCategory && form.category && form.authorBio && form.description;
      ...
      <div className="bg-gradient-to-r from-[#b11e22] to-[#921417]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="font-serif text-xl font-bold text-white">上传原创作品</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-amber-100/50 overflow-hidden">
          ...
          <div className="p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="sm:col-span-2">
              ...
              <div ref={cascaderRef} className="relative">
                <div
                  onClick={() => setCascaderOpen(!cascaderOpen)}
                  className={`w-full px-4 py-2.5 border rounded-lg text-sm cursor-pointer flex items-center justify-between transition-colors ${
                    cascaderOpen
                      ? 'border-[#b11e22] ring-2 ring-[#b11e22]/20'
                      : 'border-stone-200 hover:border-stone-300'
                  }`}
                >
                  <span className={form.subCategory && form.category ? 'text-stone-800' : 'text-stone-400'}>
                    {form.subCategory && form.category
                      ? `${form.subCategory} / ${form.category}`
                      : '请选择分类'}
                  </span>
                  <ChevronDown
                    className={`w-4 h-4 text-stone-400 transition-transform ${cascaderOpen ? 'rotate-180' : ''}`}
                  />
                </div>
                  ...
                  {cascaderOpen && (
                    ...
                    >
                      <div className="flex min-h-[200px]">
                        {/* 一级面板 */}
                        <div className="w-1/2 border-r border-stone-100 bg-stone-50/50 overflow-y-auto">
                          {SUB_CATEGORIES.map(parent => (
                            <div
                              key={parent}
                              ...
                              className={`px-4 py-3 text-sm cursor-pointer flex items-center justify-between transition-colors ${
                                activeParent === parent
                                  ? 'bg-[#b11e22]/5 text-[#b11e22] font-medium'
                        ...
                        <div className="w-1/2 overflow-y-auto">
                          {activeParent && CATEGORY_OPTIONS[activeParent]?.map(child => (
                            <div
                              key={child}
                              onClick={() => {
                                setForm({ ...form, subCategory: activeParent, category: child });
                                setCascaderOpen(false);
                                setActiveParent(null);
                              }}
                              className={`px-4 py-3 text-sm cursor-pointer transition-colors ${
                                form.subCategory === activeParent && form.category === child
                                  ? 'bg-[#b11e22]/5 text-[#b11e22] font-medium'
                                  : 'text-stone-600 hover:bg-stone-50'
                              }`}
                            >
          ...
          {/* Form Footer */}
          <div className="px-6 sm:px-8 py-5 bg-stone-50 border-t border-stone-100 flex justify-between">
            <button
              onClick={onBack}
            ...
            <div className="flex space-x-3">
              <button
                className="px-6 py-2.5 text-sm text-stone-600 hover:text-stone-800 border border-stone-200 rounded-lg hover:bg-white transition-colors cursor-pointer flex items-center space-x-2"
              >
              ...
              <button
                ...
                className="px-8 py-2.5 bg-[#b11e22] text-white text-sm font-medium rounded-lg hover:bg-[#921417] transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center space-x-2"
              >
